...
def keyword_lookup(q, input_soup, source, black_file, white_file,kw_black,kw_white):
    ...
    # Cleans up the source url.
    source_tmp = source.split('.')
    source_name = ''
    for i in range(1,len(source_tmp)):
        source_name += (source_tmp[i] + '.')
        ...
        for word in keyword_black:
            if headline_found == False:
                if word in headline.text:
                    try:
                        print headline.text
                        black_file[source_name + '-' + str(time.time())] = headline.text
                    except:
                        pass
                    headline_found = True
        for word in keyword_white:
            if headline_found == False:
                if word in headline.text:
                    try:
                        print headline.text
                        white_file[source_name + '-' + str(time.time())] = headline.text
                    except:
                        pass
                    headline_found = True
    if q.empty():
